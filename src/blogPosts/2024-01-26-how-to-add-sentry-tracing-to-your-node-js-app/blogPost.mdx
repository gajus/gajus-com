...
## Major gotcha with mixing @sentry/core and @sentry/opentemetry
...
If you can, I would advise to just use `@sentry/node`, but the problem is that `@sentry/node` does not export all of the same tracing methods that `@sentry/core` does.
