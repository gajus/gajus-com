---
description: >-
  Run-through of how to train OpenAI models with custom datasets.
...
I wanted to validate to what extent is fine-tuning OpenAI models worth it.

## Process

The general idea is very simple:

1. Prepare training data that describes system message, user message and expected assistant respose.
...
```json
{
  "messages": [
    ...
    {
      "content": "{\"city\":\"New York\",\"country\":\"United States\"}",
      "role": "assistant"
    }
  ]
}
...
I used 200 messages in the learning sample (somewhere between 50 and 100 is enough apparently).

Fine-tuning took about 20 minutes and didn't cost more than USD 5.
...
Needless to say, these edge cases were not present in the training dataset.
...
After fine-tunning, `gpt-3.5-turbo` model performed just as a good as `gpt-4-turbo`.
...
However, it still takes anywhere from 300ms to 500ms to produce a response. In contrast, we get 100-200ms with [Groq](https://groq.com/) without fine-tunning.
...
So the cost of using the fine-tuned model is about 6 times higher than the cost of using the `gpt-3.5-turbo` model and about 3 times lower than the cost of using the `gpt-4-turbo` model. However, fine-tuned model requires far fewer system prompt tokens to produce a response. Continuing with our example, using fine-tuned model my system prompt is 300 tokens long, while I need system prompt of approx. 1,500 tokens to get equivalent results with `gpt-4-turbo`.

Let's say that I need to make 10,000 invocations:

* With `gpt-3.5-turbo` (fine-tuned) it would cost me USD 9 ( ((300*10,000)/1,000,000)*3 = 9 )
* With `gpt-4-turbo` it would cost me USD 150 ( ((1,500*10,000)/1,000,000)*10 = 150 )
...
When I started reading about fine-tunning, the thing that threw me off is that a lot of the examples insisted that _the training prompt should reflect how the model is going to be used_.

In retrospect, I misunderstood this as "the training dataset should use the same prompt as the system prompt that we are trying to optimize", which in my case was the prompt that I used to build the training dataset.

If you refer back to the [process](#process), what happened was that I built a prompt that was already able to produce the desired output. It was a long prompt, but it was able to produce the desired output. I then used that very prompt to create a dataset for fine-tunning the module. The problem is that I also included that very prompt as the system prompt in the training dataset. The fine-tuned model worked fine, but it was expecting the origianl system prompt, which largely defeated the intent (reducing token usage).
...
Then over time you want manually review all entries in the table and mark included those that cover various edge cases. This will allow you to iterate over various edge cases, improve model, and over time keep adding new edge cases to the training sample.