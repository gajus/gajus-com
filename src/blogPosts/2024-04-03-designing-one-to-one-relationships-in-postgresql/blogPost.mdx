...
### Option 2: Use a unique index and a naming convention
...
### Option 3: Identify the associated row in the table that owns the relationship
...
There is no perfect way to enforce a one-to-one relationship between two tables in SQL. Historically, I've used the third approach (identifying the associated row in the table that owns the relationship) because it makes the relationship explicit and easy to understand. However, it is not foolproof. Whatever approach you choose, the most important thing is to pick one and stick with it. Consistency is key.